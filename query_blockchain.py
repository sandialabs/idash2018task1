...
class TimestampIterator:
    def __init__(self, nodenum):
        ...
        self.api = api
        self.numrecords = self.api.liststreams("node%dtimestamps" % self.nodenum)[0]['items']
        self.recordnum = 0
    ...
    def __getitem__(self,i):
        return int(self.api.liststreamkeyitems("node%dtimestamps" % self.nodenum, '*', False, 1, i, True)[0]['key'])
    ...
    def next(self):
        if self.recordnum == self.numrecords:
            ...
            self.recordnum += 1
            return int(self.api.liststreamkeyitems( "node%dtimestamps" % self.nodenum, '*', False, 1, self.recordnum-1, True)[0]['key'])
...
def interval_ids(nodenum, starttimestamp, endtimestamp):
        ...
        #print 'nodenum %d, lower %s' % (nodenum, lower_bound)
        #print 'nodenum %d, upper %s' % (nodenum, upper_bound)
        ...
        if upper_bound - lower_bound == 0:
            ...
            return {'UID_%d_%d' % (nodenum, x) for x in xrange(int(resulti.split('_')[2]), int(resultj.split('_')[2])+1)}

#==== main query logic ====

def loaditem(itemid):
    global api
    answer = api.liststreamkeyitems("logdata", '%s' % itemid)
    if len(answer) > 0:
        assert len(answer) == 1
        return json.loads(binascii.unhexlify(answer[0]['data']))
    else:
        return None
...
def query(clauses, timestamp_range, sortby, descendingp):
    ...
    if not clauses:
            ...
            if timestamp_range and (len(timestamp_matches) <= match_cts[0][0]):
                    ...
                    matches = {binascii.unhexlify(x['data']) for x in api.liststreamkeyitems( match_cts[0][1].split('_')[0], \
...
if __name__=='__main__':
    ...
    query(clauses, timestamprange, args.sortby.lower() if args.sortby else None, args.descendingp)
